...
const DownloadButtons = () => {
  ...
  const handleClick = (
    ...
    cb: (e: React.MouseEvent<HTMLButtonElement>, formattedName: string) => void
  ) => {
    if (!name) {
  ...
  const formatName = (fullName: string) => {
    return fullName
      .split(" ")
      .map((word: string, index: number) => (index == 0 ? word : word[0]))
      .join("");
  };

  const exportToStudySetCsv = (
    e: React.MouseEvent<HTMLButtonElement>,
    ...
    let vocabWordsCsv = favoriteWords.reduce((acc: string[][], word) => {
  ...
  const exportToReportCsv = (
    ...
    let vocabWordsCsv = favoriteWords.reduce((acc: string[][], word) => {
      ...
      acc.push([term, difficulty ? String(difficulty) : ""]);
      return acc;
    }, []);
